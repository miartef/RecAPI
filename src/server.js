...
const atualizarTreino = async (req, res) => {
  ...
  if (isNaN(id)) {
    return res.status(400).json({
      erro: "ID do treino é obrigatório e deve ser numérico"
    });
  }
...
app.post("/exercicios", async (req, res) => {
  ...
  if (!nome || !grupoMuscular) {
    return res.status(400).json({
      erro: "Nome e grupoMuscular são obrigatórios"
    });
  }
...
app.post("/treinos/vincular", async (req, res) => {
  ...
  if (isNaN(treinoId) || isNaN(exercicioId)) {
    return res.status(400).json({
      erro: "treinoId e exercicioId são obrigatórios e devem ser números"
    });
  }
  ...
  try {
    ...
    if (error.code === "P2002") {
      return res.status(409).json({
        erro: "Este exercício já está vinculado a este treino"
      });
    }

    if (error.code === "P2003") {
      return res.status(404).json({
        erro: "Treino ou exercício informado não existe"
      });
    }
...
app.get("/treinos/:id/exercicios", async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({
      erro: "Erro ao listar exercícios do treino"
    });
  }
});
...
app.delete("/treinos/:id/exercicios/:exercicioId", async (req, res) => {
  ...
  try {
    ...
    if (error.code === "P2025") {
      return res.status(404).json({
        erro: "Exercício não está vinculado a este treino"
      });
    }

    console.error(error);
    res.status(500).json({
      erro: "Erro ao remover exercício do treino"
    });
  }
});
